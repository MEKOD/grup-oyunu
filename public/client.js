// client.js - SON DÜZELTME

// DOMContentLoaded'ı KORUYORUZ, en garanti yöntem bu.
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Olay Dinleyicileri (Daha Güvenli Hâle Getirildi) ---
    if (createRoomBtn) {
        createRoomBtn.addEventListener('click', () => {
            const name = nameInput.value.trim();
            if (!name) {
                loginError.textContent = 'Takma ad girmek zorunlu!';
                return;
            }
    ...
    if (showJoinBtn) {
        showJoinBtn.addEventListener('click', () => {
            const name = nameInput.value.trim();
            if (!name) {
                loginError.textContent = 'Önce takma adını girmelisin!';
                return;
            }
            loginError.textContent = '';
    ...
    if (joinRoomBtn) {
        joinRoomBtn.addEventListener('click', () => {
            ...
            if (!roomId) {
                joinError.textContent = 'Oda kodu girmelisin!';
                return;
            }
            joinError.textContent = '';
            socket.emit('joinRoom', { roomId, name: nameInput.value.trim() });
        });
    }
    ...
    socket.on('newTurn', ({ player, task }) => {
        updatePlayerDisplays(gameState.players, player.id);
        
        let content = `
            <div class="task-card">
                <p class="task-type">${task.type.toUpperCase()}</p>
        ...
        if (player.id === gameState.myId) {
            ...
            content += `<p class="subtitle">Görevi yapıyor mu?</p>
            <div class="vote-btn-group">
                <button class="btn btn-success vote-btn" data-vote="yapildi">✅ İkna Oldum</button>
                <button class="btn btn-fail vote-btn" data-vote="yapilmadi">❌ İkna Olmadım</button>
            </div>`;
        }
