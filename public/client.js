...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleDirectJoin() { const path = window.location.pathname; const match = path.match(/\/join\/([A-Z0-9]{6})/); if (match) { const roomId = match[1]; const name = localStorage.getItem('playerName') || prompt("Oyuna katılmak için takma adını gir:"); if (name && name.trim()) { localStorage.setItem('playerName', name.trim()); nameInput.value = name.trim(); socket.emit('joinRoom', { roomId, name: name.trim() }); } } }
    handleDirectJoin();
});