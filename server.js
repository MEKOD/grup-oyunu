// server.js - STABİLİTE GÜNCELLEMESİ (TOKEN SİSTEMİ)
...
import crypto from 'crypto'; // <-- Token üretmek için eklendi
...
const io = new Server(server, {
  ...
  transports: ['websocket', 'polling'] // Bağlantıyı stabil hale getir
});
...
io.on('connection', (socket) => {
    console.log(`[BAĞLANTI] Kullanıcı bağlandı: ${socket.id}`);

    // --- YENİ: Token ile yeniden bağlanma mantığı ---
    socket.on('reconnectWithToken', ({ token }) => {
        ...
        if (foundPlayer) {
            console.log(`[YENİDEN BAĞLANMA] ${foundPlayer.name} oyuncusu token ile bulundu.`);
            foundPlayer.id = socket.id; // Socket ID'sini güncelle
            socket.join(foundRoomId);
            // Oyuncuya odanın son durumunu göndererek senkronize et
            socket.emit('reconnectSuccess', rooms[foundRoomId]);
            // Odadaki diğer herkese güncel oyuncu listesini gönder
    ...
    const createAndJoin = (isHost, name, roomId = null) => {
        ...
        // --- KRİTİK: Token'ı SADECE o oyuncuya gönder ---
        ...
        console.log(`[${isHost ? 'ODA KURULDU' : 'KATILIM'}] ${name} oyuncusu ${roomId} odasına katıldı.`);
    };

    socket.on('createRoom', ({ name }) => createAndJoin(true, name));
    socket.on('joinRoom', ({ roomId, name }) => createAndJoin(false, name, roomId));

    // ... (Diğer tüm olaylar 'startGame', 'passTask' vb. aynı kalıyor, hiç bir değişiklik yok)
